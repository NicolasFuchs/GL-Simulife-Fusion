...
public class MoveQueen extends Move {
  ...
  public MoveQueen(Creature[][] game, Creature c, LinkedList<Creature> list) {
    super(game, c, list);
    rd = new Random();
    this.c = c;
    this.game = game;
  ...
  protected void createPossibleMoves() {
    ...
    for (int i = 0; i < possibleGrid.length; i++) {
      for (int j = 0; j < possibleGrid[i].length; j++) {
        System.out.print(possibleGrid[i][j] ? "T " + "[33mYELLOW" : "F ");
      }
      System.out.println();
    }
